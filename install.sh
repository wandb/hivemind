...
case "$OS-$ARCH" in
  Darwin-arm64)
    ...
    ;;
  Darwin-x86_64)
    die "Intel Macs are not currently supported. The hivemind binary is built for Apple Silicon only. Please install via the Homebrew formula instead: brew trust --formula wandb/taps/hivemind && brew install wandb/taps/hivemind"
    ;;
  Linux-x86_64)
